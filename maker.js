...
const base = 'C:/Users/Usuario/Desktop/Huellas_Salud_Backend';

['src/config', 'src/data', 'src/routes', 'src/db', 'uploads'].forEach(d => {
...
fs.writeFileSync(path.join(base, '.env'), 'PORT=3000\nJWT_SECRET=super_secret_huellas_salud_key_2026\nDATABASE_URL=\nNODE_ENV=development\n');
fs.writeFileSync(path.join(base, '.gitignore'), 'node_modules\n.env\nuploads/*\n!.gitkeep\n');
fs.writeFileSync(path.join(base, 'uploads/.gitkeep'), '');

// seedData.js
const seedObj = {
  users: [
    { id: 1, documentNumber:  123456789, email: admin@huellassalud.com, name: Armando, lastName: Puentes, role: ADMIN, password: password123 },
    { id: 2, documentNumber: 987654321, email: valeria@huellassalud.com, name: Valeria, lastName: Gómez, role: VETERINARIO, password: password123 }
  ],
  pets: [
    {
      idPet: 1,
      ...
      sex: Macho,
      age: 4,
      mediaFile: { fileName: max.jpg, contentType: image/jpeg, attachment:  },
 medicalHistory: [
 {
 ...
 sex: Hembra,
 age: 2,
 mediaFile: { fileName: luna.jpg, contentType: image/jpeg, attachment:  },
      medicalHistory: [
        {
  ...
  products: [
    { idProduct: 1, name: Pro Plan Adulto Razas Medianas 3kg, category: Alimento, animalType: Perro, description: Alimento completo y balanceado, price: 85000, mediaFile: null },
    { idProduct: 2, name: Juguete Kong Classic Medium, category: Juguetes, animalType: Perro, description: Juguete de caucho súper duradero, price: 45000, mediaFile: null },
    { idProduct: 3, name: Bravecto Antipulgas Canino 10-20kg, category: Medicinas, animalType: Perro, description: Comprimido masticable antipulgas, price: 120000, mediaFile: null }
  ],
  invoices: [
    {
      idInvoice: F-146,
  ...
  announcements: [
    {
      ...
      description: ¡Gran Jornada de Vacunación este Fin de Semana! Descuentos del 20% en vacunas.,
...
fs.writeFileSync(path.join(base, 'src/data/seedData.js'), 'module.exports = ' + JSON.stringify(seedObj, null, 2) + ';\n');

// db.js
fs.writeFileSync(path.join(base, 'src/config/db.js'), 
...
// auth.routes.js
fs.writeFileSync(path.join(base, 'src/routes/auth.routes.js'), 
...
router.post('/user/login', (req, res) => {
  ...
  return res.status(200).json({
    token,
    access_token: token,
    data: { id: user.id, documentNumber: user.documentNumber, email: user.email, name: user.name, lastName: user.lastName, role: user.role, token }
  });
});
...
// users.routes.js
fs.writeFileSync(path.join(base, 'src/routes/users.routes.js'), 
const express = require('express');
const seedData = require('../data/seedData');
const router = express.Router();

router.get('/user/list-users', (req, res) => {
  const formatted = seedData.users.map(u => ({ data: { name: u.name, lastName: u.lastName, role: u.role, documentNumber: u.documentNumber } }));
  res.json(formatted);
});
...
// pets.routes.js
fs.writeFileSync(path.join(base, 'src/routes/pets.routes.js'), 
const express = require('express');
const seedData = require('../data/seedData');
const router = express.Router();
...
// products.routes.js
fs.writeFileSync(path.join(base, 'src/routes/products.routes.js'), 
const express = require('express');
const seedData = require('../data/seedData');
const router = express.Router();
...
// invoices.routes.js
fs.writeFileSync(path.join(base, 'src/routes/invoices.routes.js'), 
const express = require('express');
const seedData = require('../data/seedData');
const router = express.Router();
...
fs.writeFileSync(path.join(base, 'src/routes/announcements.routes.js'), 
const express = require('express');
...
router.get('/announcement/list-announcements', (req, res) => {
  const formatted = seedData.announcements.map(ann => ({
    data: ann,
    meta: { nameUserCreated: ann.nameUserCreated, emailUserCreated: ann.emailUserCreated, roleUserCreated: ann.roleUserCreated }
  }));
  res.json(formatted);
});
...
// schema.sql
fs.writeFileSync(path.join(base, 'src/db/schema.sql'), 
...
// index.js
fs.writeFileSync(path.join(base, 'src/index.js'), 
...
console.log('BUILD_COMPLETE_SUCCESS');
